...
toc: true
permalink: /vue-mvvm
title: 手动实现 MVVM 架构中的双向绑定
tags: JavaScript Vue MVVM
desc: 随着 Web 应用的发展, 直接操作DOM的应用已渐行渐远, 取而代之的是时下最流行的MVVM框架. MVVM框架的双向绑定使得开发效率大大提高; 在实现这些双向绑定时, 目前大多数的方案都是采用Object.defineProperty和Object.defineProperties方法. 本篇文章主要参考了同仁的实现, 并在此基础上加上自己的理解注释, 希望能够对阅读者有所帮助.
---
...
这里通过递归遍历保证了每个节点及子节点都会解析编译到，包括了 双大括号 表达式声明的文本节点。指令的声明规定是通过特定前缀的节点属性来标记，如<span v-text="content" other-attr中v-text便是指令，而other-attr不是指令，只是普通的属性。
